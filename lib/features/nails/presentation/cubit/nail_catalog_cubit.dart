...
class NailCatalogCubit extends Cubit<NailCatalogState> {
  ...
  Future<void> loadDesigns() async {
    emit(
      state.copyWith(
        status: NailCatalogStatus.loading,
        page: 1,
        clearError: true,
      ),
    );
    try {
      final designsPage = await _repository.getNailDesigns(
        page: 1,
        pageSize: _pageSize,
        filters: state.filters,
      );
      final categoryTypes = state.categoryTypes.isEmpty
          ? await _repository.getCategoryTypes()
          : state.categoryTypes;
      emit(
        state.copyWith(
      ...
      emit(
        state.copyWith(
          status: NailCatalogStatus.error,
