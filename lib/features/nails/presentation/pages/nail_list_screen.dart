...
class _NailListViewState extends State<_NailListView> {
  ...
  void _onSearchChanged(String query) {
    ...
    _debounce = Timer(const Duration(milliseconds: 500), () {
      ...
      _activeCategoryName = null; // Reset category name override on search change
        ...
        return RefreshIndicator(
          ...
          child: CustomScrollView(
            ...
            slivers: [
              ...
              if (state.filters.isNotEmpty || _activeCategoryName != null)
                SliverToBoxAdapter(
                  child: Padding(
              ...
              else if (state.filters.isNotEmpty || _activeCategoryName != null)
  ...
  Widget _buildFilteredGrid(BuildContext context, NailCatalogState state) {
    ...
    // Filter locally if viewing high-level collections
    List<NailDesignModel> designsToDisplay = state.designs;
    if (_activeCategoryName == 'Theo mùa' || _activeCategoryName == 'Seasonal') {
      designsToDisplay = state.designs.where((d) {
        return d.categories.any((c) {
          final typeName = c.categoryTypeName.toLowerCase();
          return typeName == 'theme' || typeName == 'season' || typeName == 'mùa';
        });
      }).toList();
    } else if (_activeCategoryName == 'Theo phong cách' || _activeCategoryName == 'By Style') {
      designsToDisplay = state.designs.where((d) {
        return d.categories.any((c) {
          final typeName = c.categoryTypeName.toLowerCase();
    ...
    } else if (_activeCategoryName == 'Theo tông da' || _activeCategoryName == 'By Skin Tone') {
      designsToDisplay = state.designs.where((d) {
        return d.categories.any((c) {
          final typeName = c.categoryTypeName.toLowerCase();
    ...
    } else if (_activeCategoryName == 'Theo dịp' || _activeCategoryName == 'By Occasion') {
      designsToDisplay = state.designs.where((d) {
        return d.categories.any((c) {
          final typeName = c.categoryTypeName.toLowerCase();
        ...
        SliverToBoxAdapter(
          child: NailCollectionSection(
            ...
            onDesignTap: (design) =>
                context.go('/nails/${design.nailDesignId}'),
            onSeeAll: () {
              // Clear server filters so we fetch all designs, then we filter locally!
              context.read<NailCatalogCubit>().applyFilters(const NailFilters());
              _activeCategoryName = 'Theo mùa';
              setState(() {});
            },
          ),
        ),
      ...
      sections.add(
        SliverToBoxAdapter(
          child: NailCollectionSection(
            ...
            onSeeAll: () {
              context.read<NailCatalogCubit>().applyFilters(const NailFilters());
              _activeCategoryName = isVi ? 'Theo phong cách' : 'By Style';
              setState(() {});
            },
          ),
        ),
      ...
      sections.add(
        SliverToBoxAdapter(
          child: NailCollectionSection(
            ...
            onSeeAll: () {
              context.read<NailCatalogCubit>().applyFilters(const NailFilters());
              _activeCategoryName = isVi ? 'Theo tông da' : 'By Skin Tone';
              setState(() {});
            },
          ),
        ),
      ...
      sections.add(
        SliverToBoxAdapter(
          child: NailCollectionSection(
            ...
            onSeeAll: () {
              context.read<NailCatalogCubit>().applyFilters(const NailFilters());
              _activeCategoryName = isVi ? 'Theo dịp' : 'By Occasion';
              setState(() {});
            },
          ),
        ),
        ...
        SliverToBoxAdapter(
          child: NailCollectionSection(
            ...
            onSeeAll: () {
              ...
              if (match.isNotEmpty) {
                final catId = match.first.categoryTypeId;
                ...
                // If it is a fallback category (like "Được đề xuất"), we clear filters to show all designs
                context.read<NailCatalogCubit>().applyFilters(const NailFilters());
              }
              _activeCategoryName = entry.key;
              setState(() {});
            },
          ),
        ),
  ...
  Future<void> _openFilters(
    ...
    if (filters != null && mounted) {
      ...
      _activeCategoryName = null; // Reset category name override when using filter sheet
      setState(() {});
    }
  }
  ...
  String _getFilterActiveText(BuildContext context, NailCatalogState state) {
    ...
    if (_activeCategoryName != null) {
      final name = (_activeCategoryName == 'Theo mùa' || _activeCategoryName == 'Seasonal')
          ? s.seasonalTitle
          : _activeCategoryName;
      return isVi ? 'Đang xem danh mục "$name"' : 'Viewing category "$name"';
    }
    ...
    if (filters.categoryIds.isNotEmpty) {
      final catNames = state.categoryTypes
          .where((c) => filters.categoryIds.contains(c.categoryTypeId))
          .map((c) => c.name)
          .join(', ');
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              'Giá từ ${PriceFormatter.format(design.minPrice).replaceAll(' VNĐ', '')} • ${design.nailVariants.length} phiên bản • $reviewsCountStr đánh giá',
              style: TextStyle(
                fontSize: 13,
