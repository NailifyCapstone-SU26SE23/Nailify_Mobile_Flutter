...
class _ProfilePageState extends State<ProfilePage> {
  ...
  void initState() {
    ...
    for (var note in ProfileMockData.personalNotes) {
      _noteControllers[note.title] = TextEditingController(text: note.content ?? '');
    }
  }
  ...
  Future<bool> _updateProfile(String firstName, String lastName, String phone, File? imageFile) async {
    try {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Hồ sơ cá nhân', style: TextStyle(fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
        centerTitle: true,
             ...
             PersonalityStyleSection(
              ...
              onToggle: (id) {
                setState(() {
                  ...
                  } else if (_selectedPersonalities.length < ProfileMockData.maxPersonalitySelections) {
            ...
            children: [
              Container(
                ...
                child: CircleAvatar(
                  ...
                  onBackgroundImageError: avatarUrl.isNotEmpty ? (e, s) => debugPrint('Lỗi tải ảnh: $e') : null,
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        const Icon(Icons.stars, size: 16, color: Colors.yellowAccent),
                        const SizedBox(width: 8),
          ...
          Row(
            ...
            children: [
              Container(
                ...
                child: Text('Trạng thái: $status', style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w600)),
              ),
              ElevatedButton.icon(
                onPressed: _showUpdatePopup,
                icon: const Icon(Icons.edit, size: 16, color: AppColors.primary),
                label: const Text('Cập nhật', style: TextStyle(color: AppColors.primary, fontWeight: FontWeight.bold)),
  ...
  void _showUpdatePopup() {
    final firstNameController = TextEditingController(text: _profileData?['firstName']?.toString() ?? '');
    final lastNameController = TextEditingController(text: _profileData?['lastName']?.toString() ?? '');
    final phoneController = TextEditingController(text: _profileData?['phone']?.toString() ?? '');
        ...
        return StatefulBuilder(
          builder: (context, setStateDialog) {
            ...
            return AlertDialog(
              backgroundColor: Colors.white,
              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
              title: const Text('Cập nhật hồ sơ', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 20)),
              content: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                        ...
                        children: [
                          CircleAvatar(
                            ...
                            backgroundImage: selectedImage != null
                                ...
                                : null,
                            onBackgroundImageError: avatarUrl.isNotEmpty ? (e, s) => debugPrint('Lỗi ảnh popup') : null,
                    ...
                    const SizedBox(height: 24),
                    _buildTextField(firstNameController, 'Tên (First Name)', Icons.person_outline),
                    const SizedBox(height: 16),
                    _buildTextField(lastNameController, 'Họ (Last Name)', Icons.badge_outlined),
                    const SizedBox(height: 16),
                    _buildTextField(phoneController, 'Số điện thoại', Icons.phone_outlined, isNumber: true),
                  ],
                ),
              ),
              actions: [
                ...
                ElevatedButton(
                  onPressed: isSubmitting ? null : () async {
                    ...
                    if (success) {
                      ...
                      if (dialogContext.mounted) setStateDialog(() => isSubmitting = false);
                    }
                  },
