 ...
 *
 * Port of nail_desktop_app/overlay.py NailOverlayRenderer._render_seg_5class_overlay.
 *
 * Pipeline for each detection:
 *   1. Choose direction (v_long) = forward_vector (MediaPipe) ?? pca_direction ?? default up.
 *   2. Compute nail-bed width by projecting nail_bed_polygon onto v_short.
 *   3. Compute h_mapped = w_bed * design_aspect_ratio.
 *   4. Compute 4 destination points (cuticle_L, cuticle_R, tip_L, tip_R) on the frame.
 *   5. Compute 4 source points (opaque bbox of design) -> design_corners.
 *   6. findHomography(src, dst) + warpPerspective.
 *   7. Build hybrid mask: full nail polygon UNION rect from 40% along v_long -> tip.
 *   8. AND with design mask.
 *   9. Alpha-blend the warped design RGB onto the frame where mask > 0.
 */
...
object NailOverlayEngine {
    ...
    /** Fraction along v_long at which the tip rectangle starts (relative to
     *  full nail length).  Below this fraction the natural nail polygon
     *  defines the boundary (preserving cuticle curve); above it, the design
     *  flows into the "infinite tip" rectangle.  Matches overlay.py. */
    private const val TIP_RECT_START = 0.4f

    /**
     * Render every detection onto `frame` using the given per-finger design
     * map. Each detection looks up its design by [NailDetection.designAssetPath];
     * if the path is null or the asset is not loaded, [defaultDesignAsset] is
     * used as a fallback. If even the fallback is missing, the detection is
     * skipped.
     *
     * The frame is modified in place and returned for convenience.
     *
     * @param designs             Map of asset path -> [LoadedDesign] (from
     *                            MainViewModel design cache).
     * @param defaultDesignAsset  Asset path used as fallback (e.g. "ballerina.png").
     */
    fun render(
        frame: Bitmap,
        ...
        for (det in detections) {
            try {
                val design = det.designAssetPath
                    ?.let { designs[it] }
                    ?: defaultDesign
    ...
    private fun renderOne(
        ...
        // 1. Direction (v_long, unit). PCA direction from polygon is the SPATIAL
        //    ANCHOR (don't move it). MediaPipe forwardVector is a ROTATION HINT
        //    used only to flip PCA's 180° ambiguity when needed. Never let
        //    MediaPipe override the spatial frame — otherwise the design drifts
        //    off the nail when MediaPipe briefly flips at ~90° rotation.
        ...
        if (rotationHint != null) {
            ...
            if (hintMag > 1e-9f) {
                ...
                if (dot < 0f) {
                    vLong = PointF(-vLong.x, -vLong.y)
                }
            }
        }
        // 2. Perpendicular (left -> right) of v_long.
        val vShort = PointF(-vLong.y, vLong.x)

        // 3. Nail-bed width: project nail_bed_polygon onto v_short.
        ...
        // 4. Full-nail length (min/max along v_long) — used for the tip rect.
        ...
        // 5. Aspect ratio preserves the design's aspect.
        val aspectRatio = design.aspectRatio
        val hMapped = wBed * aspectRatio
        val tipProj = minLong + hMapped

        // 6. Destination points (on the frame): [cuticle_L, cuticle_R, tip_R, tip_L]
        ...
        // 7. Source points (opaque bbox of the design).
        //    Design is upright: Base = bottom (y = oy + oh), Tip = top (y = oy)
        val ob = design.opaqueBounds
        val src = arrayOf(
            PointF(ob.left,  ob.bottom),   // base-left  = design BL
            PointF(ob.right, ob.bottom),   // base-right = design BR
            PointF(ob.right, ob.top),      // tip-right  = design TR
            PointF(ob.left,  ob.top),      // tip-left   = design TL
        )
        ...
        if (designMask == null) {
            Log.w(TAG, "5-class warp produced empty design_mask for cls=${det.clsName}")
            return
        }
        ...
        if (nailMask == null) return

        val finalMask = andMask(designMask, nailMask) ?: return

        // 10. Blend warped RGB onto the frame.
    ...
    /**
     * Solve a 3x3 homography that maps src (4 points) to dst (4 points)
     * using the standard DLT formulation.  Returns the row-major 3x3 matrix.
     */
    private fun findHomography(src: Array<PointF>, dst: Array<PointF>): FloatArray? {
        if (src.size != 4 || dst.size != 4) return null
        // Build the 8x8 linear system Ah = b.
        ...
        // h is length-8 [h00..h20, h01..h21]; h22 = 1.
        return floatArrayOf(
            h[0], h[1], h[2],
            h[3], h[4], h[5],
            h[6], h[7], 1f,
    ...
    /**
     * Gauss-Jordan elimination on an n x n matrix augmented with a length-n
     * right-hand-side vector.  Returns the n-length solution.
     */
    ...
    /**
     * Apply a 3x3 homography to every pixel of `src`, producing a new RGBA
     * bitmap of (outW, outH) using bilinear interpolation and constant-zero
     * border (matches cv2.warpPerspective(BORDER_CONSTANT, (0,0,0,0))).
     */
    private fun warpPerspective(src: Bitmap, h: FloatArray, outW: Int, outH: Int): Bitmap {
        ...
        // For each dst pixel, compute inverse-mapped src coords.
    ...
    private fun alphaChannelMask(rgba: Bitmap, w: Int, h: Int): Bitmap? {
        ...
        // Probe center pixel to see if anything was warped into the frame.
        val centerAlpha = if (mask.getPixel(w / 2, h / 2) == 0) {
            // Sample a few locations to be safe (the center may fall outside the warped region).
        ...
        return mask
        // centerAlpha local was kept to keep IDE happy without unused-warning suppression
        // The actual presence check uses getPixel(0,0)-style sampling above.
    }
    ...
    private fun createHybridNailMask(
        ...
        // 1. Draw the natural nail polygon (preserves the cuticle curve).
        val path = Path()
        ...
        // 2. Draw the "infinite tip" rectangle starting at 40% of the natural nail length.
    ...
    /**
     * Blend `warped` (RGBA) onto `frame` using `mask` (ALPHA_8).
     *   - mask==0 -> keep frame unchanged.
     *   - mask==max -> replace frame with `warped`'s RGB (blend mode applied).
     */
    ...
    private fun blendPreserveLighting(src: Int, dst: Int, maskAlpha: Int): Int {
        // Use the destination's luminance to scale the source; preserves highlights/shadows.
        val dstR = Color.red(dst)
        val dstG = Color.green(dst)
        val dstB = Color.blue(dst)
        val lum = 0.114f * dstR + 0.587f * dstG + 0.299f * dstB
        // Reference mean ~ 128; ratio is per-pixel.
